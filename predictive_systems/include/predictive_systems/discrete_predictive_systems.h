#ifndef   PREDICTIVE_SYSTEMS__DISCRETE_PREDICTIVE_SYSTEMS__H
#define   PREDICTIVE_SYSTEMS__DISCRETE_PREDICTIVE_SYSTEMS__H

#include <memory>
#include <Eigen/Core>
#include <ros/node_handle.h>
#include <predictive_systems/symbols.h>
#include <predictive_systems/predictive_systems.h>
...
template< int S,          // State Dimension
          ...
          int MaxNp= Np,   // g.t I if specified, it pre-allocates the max usable memory in the stack
          int MaxNc= Nc>   // g.t O if specified, it pre-allocates the max usable memory in the stack
struct DiscretePredictiveModelArgs : public BasePredictiveModelArgs
{
  typedef std::shared_ptr<DiscretePredictiveModelArgs> Ptr;
  typedef std::shared_ptr<DiscretePredictiveModelArgs const> ConstPtr;

  DiscretePredictiveModelArgs() = default;
  virtual ~DiscretePredictiveModelArgs() = default;

  typename PredictiveModelSymbols<S,I,O,Np,Nc,MaxS,MaxI,MaxO,MaxNp,MaxNc>::MatrixA A;
  typename PredictiveModelSymbols<S,I,O,Np,Nc,MaxS,MaxI,MaxO,MaxNp,MaxNc>::MatrixB B;
  typename PredictiveModelSymbols<S,I,O,Np,Nc,MaxS,MaxI,MaxO,MaxNp,MaxNc>::MatrixC C;
  typename PredictiveModelSymbols<S,I,O,Np,Nc,MaxS,MaxI,MaxO,MaxNp,MaxNc>::MatrixD D;
};
...
template< int S,          // State Dimension
          ...
          int MaxNp= Np,   // g.t I if specified, it pre-allocates the max usable memory in the stack
          int MaxNc= Nc>   // g.t O if specified, it pre-allocates the max usable memory in the stack
class DiscretePredictiveModel : public BasePredictiveModel
{

public:
  using Symbols = PredictiveModelSymbols<S,I,O,MaxS,MaxI,MaxO>;
  ...
  DiscretePredictiveModel()=default;
  virtual ~DiscretePredictiveModel() = default;
  DiscretePredictiveModel(const DiscretePredictiveModel&) = delete;
  DiscretePredictiveModel& operator=(const DiscretePredictiveModel&)= delete;
  DiscretePredictiveModel(DiscretePredictiveModel&&) = delete;
  DiscretePredictiveModel& operator=(DiscretePredictiveModel&&) = delete;
   ...
   */
  DiscretePredictiveModel(const DiscretePredictiveModelArgs<S,I,O,MaxS,MaxI,MaxO>& args);
   ...
   */
  virtual int setMatrices (const BasePredictiveModelArgs& args, std::string& msg) override;
...
using DiscretePredictiveModelPtr = std::shared_ptr<DiscretePredictiveModel<S,I,O,MS,MI,MO> >;

typedef DiscretePredictiveModel<-1,-1,-1> DiscretePredictiveModelX;
...
#include <predictive_systems/internal/discrete_predictive_systems_impl.h>

#endif  // PREDICTIVE_SYSTEMS__DISCRETE_PREDICTIVE_SYSTEMS__H
