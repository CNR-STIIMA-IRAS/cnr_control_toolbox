...
{
  if(iAx<0 || iAx >eigen_utils::rows(values_))
  {
    throw std::runtime_error("FilteredValue<N,MaxN>::value(iAx) index out of boundaries");
  }
  const double* ptr_to_val = data() + iAx;
  return *ptr_to_val;
}
